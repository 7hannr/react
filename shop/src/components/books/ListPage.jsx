...
const ListPage = () => {
  ...
  return (
    <div className='my-5'>
      <h1 className='text-center my-4'>도서목록</h1>
      <Row className='mb-2'>
        <Col xs={8} md={5} lg={4}>
        ...
        {count >0 &&
          <Col className='text-end'>
            <Button onClick={onDeleteChecked} variant='light' size='sm'>선택도서삭제</Button>
          </Col>
        }
      </Row>
      {count > 0 ?
      <Table>
        <thead>
          <tr className='text-center table-danger'>
            <td><input onChange={onChangeAll}
              checked={chk === books.length} type="checkbox" /></td>
            <td>ID.</td>
            <td>Image</td>
            <td>제목</td>
            <td>가격</td>
        ...
        <tbody className='text-center'>
          {books.map(book =>
            <tr key={book.bid}>
              ...
              <td>{book.author}</td>
              <td>{book.fmtdate}</td>
              <td><Button onClick={() => onDelete(book)}
                variant='light' size='sm'>삭제</Button></td>
            </tr>
          )}
        </tbody>
      </Table>
