...
const CartPage = () => {
  ...
  useEffect(()=>{
    ...
    //console.log('ddddd',checkedCount);
    setChk(checkedCount);
  },[books]);
  const callAPI = async() => {
  ...
  return (
    <Row className='justify-content-center my-5'>
      <Col xs={12} md={10} lg={8}>
        ...
        <div className='mb-2'>
          <Button onClick={CheckedDelete}
          variant='light' size='sm'>선택도서삭제</Button>
        </div>
        <Table>
          <thead>
            ...
            {books.map(book=>
              <tr key={book.bid}>
                ...
                <td>
                  ...
                  value={book.qnt} size={1} className='text-end me-1'/>
                  <Button onClick={()=>onUpdateQnt(book.bid,book.qnt)}
                  variant='light' size="sm">수정</Button>
                </td>
                <td>{book.sum.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}원</td>
                <td>
                  <Button onClick={()=>onClickDelete(book.bid)}
                  variant='light' size='sm'>삭제</Button>
                </td>
              </tr>
            )}
          </tbody>
        ...
        <div className='text-end'>
          <a href="/"><Button className='me-2 ' variant='light'>쇼핑계속하기</Button></a>
          <Button onClick={onOrder} variant='light'>주문하기</Button>
        </div>
        </>
        :
