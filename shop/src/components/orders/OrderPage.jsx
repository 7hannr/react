...
const OrderPage = ({ books, setBooks }) => {
  ...
  return (
    <div>
      ...
      <Alert variant='light' className='text-end'>주문합계:{total.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}원</Alert>
      <Card>
        <Card.Header>
        ...
        <Card.Body>
          ...
          <InputGroup className='mb-2'>
            <InputGroup.Text>주문자 전화</InputGroup.Text>
            <Form.Control name="phone" value={phone} onChange={onChangeForm} />
          </InputGroup>
          <InputGroup className='mb-1'>
            <InputGroup.Text>주문자 주소</InputGroup.Text>
